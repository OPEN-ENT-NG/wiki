...
wikiNamespace.Version.prototype.comparison = function(left, right){
	...
	leftRoot.each(function(index, item){
		if($(item).hasClass('added')){
			(rightRoot as any).splice(index + added, 0, $(item.outerHTML).removeClass('added').addClass('removed')[0]);
		}
	...
	rightRoot.each(function(index, item){
		if($(item).hasClass('added')){
			(leftRoot as any).splice(index, 0, $(item.outerHTML).removeClass('added').addClass('removed')[0]);
		}
	});
